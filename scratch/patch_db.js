...
const imagePatch = {
  "gif-maker": {
    ...
    faqs: [
      { question: "Can I adjust the speed of the animation?", answer: "Yes, you can set custom millisecond delays between frames to speed up or slow down the loop." },
      { question: "Do my images get uploaded to a server?", answer: "No. The entire compilation runs in your browser cache; your images are safe." }
    ]
  },
  "heic-to-jpg": {
    ...
    faqs: [
      { question: "Is HEIC supported by Windows?", answer: "Older Windows versions cannot open HEIC files without plugins. Converting to JPG ensures they open on any device." }
    ]
  }
};
...
const devPatch = {
  "cron-generator": {
    ...
    faqs: [
      { question: "What is the crontab format standard?", answer: "The standard format has 5 fields representing: Minute, Hour, Day of Month, Month, and Day of Week." }
    ]
  },
  "password-generator": {
    ...
    faqs: [
      { question: "How does the tool ensure randomness?", answer: "It uses the Web Crypto API's random number generator, which is designed for cryptographic purposes." }
    ]
  },
  "bcrypt-generator": {
    ...
    faqs: [
      { question: "Can a Bcrypt hash be decrypted?", answer: "No. Bcrypt is a one-way cryptographic hash function. It cannot be decrypted back to plain text; it can only be verified by matching." }
    ]
  },
  "css-gradient-generator": {
    ...
    faqs: [
      { question: "Does it support alpha transparency?", answer: "Yes, you can configure the opacity of each color node using RGBA stops." }
    ]
  },
  "css-box-shadow": {
    ...
    faqs: [
      { question: "What is an inset shadow?", answer: "An inset shadow is drawn inside the boundaries of the element rather than outside, creating a recessed or pressed look." }
    ]
  },
  "color-contrast-checker": {
    ...
    faqs: [
      { question: "What is the minimum ratio for WCAG AA?", answer: "Normal text requires a contrast ratio of at least 4.5:1, while large text (18pt+) requires at least 3:1." }
    ]
  },
  "svg-optimizer": {
    ...
    faqs: [
      { question: "Will my vector lines remain sharp?", answer: "Yes, SVGs scale mathematically. Optimization only minifies code representation without degrading rendering quality." }
    ]
  },
  "pdf-image-extractor": {
    ...
    faqs: [
      { question: "Can I extract images from scanned PDFs?", answer: "If the PDF consists of scanned pages, each full page is treated as an image and can be extracted." }
    ]
  }
};

function patchFile(fileName, patchMap, dbName) {
  const filePath = path.join(detailsDir, fileName);
  let content = fs.readFileSync(filePath, 'utf8');
  
  for (const [id, data] of Object.entries(patchMap)) {
    // We want to replace the entry for id in the database
    // The format is: "id": { whyNeed: "...", ... }
    const startStr = `  "${id}": {`;
    const startIdx = content.indexOf(startStr);
    if (startIdx === -1) {
      console.error(`Could not find entry for ${id} in ${fileName}`);
      continue;
    }
    ...
    if (endIdx === -1) {
      console.error(`Could not find closing brace for ${id} in ${fileName}`);
      continue;
    }
    
    // Construct new entry
    let newEntry = `  "${id}": {\n`;
    newEntry += `    whyNeed: ${JSON.stringify(data.whyNeed)},\n`;
    newEntry += `    howWorks: ${JSON.stringify(data.howWorks)},\n`;
    newEntry += `    whenToUse: ${JSON.stringify(data.whenToUse)},\n`;
    newEntry += `    stepByStep: ${JSON.stringify(data.stepByStep)},\n`;
    newEntry += `    advantages: ${JSON.stringify(data.advantages)},\n`;
    newEntry += `    commonMistakes: ${JSON.stringify(data.commonMistakes)},\n`;
    newEntry += `    faqs: ${JSON.stringify(data.faqs)}\n`;
    newEntry += `  }`;
    
    content = content.substring(0, startIdx) + newEntry + content.substring(endIdx + 1);
  }
...
patchFile('imageDb.ts', imagePatch);
patchFile('devDb.ts', devPatch);
