...
export default async function SeriesDetailsPage({ params, searchParams }: PageProps) {
  const { seriesSlug } = await params;
  ...
  const activeTab = (queryParams?.tab as 'recent' | 'featured') || 'recent';
  const displayedGuides = activeTab === 'featured' 
    ? allGuides.filter(g => g.featured) 
    : allGuides;

  const currentPage = parseInt((queryParams?.page as string) || '1', 10);
  const pageSize = 12;
  const totalPages = Math.ceil(displayedGuides.length / pageSize) || 1;
  const startIndex = (currentPage - 1) * pageSize;
  const paginatedGuides = displayedGuides.slice(startIndex, startIndex + pageSize);

  const getPageUrl = (page: number, tab: string) => `/blog/series/${seriesSlug}?page=${page}&tab=${tab}`;
              ...
              <section className="space-y-6">
                ...
                {totalPages > 1 && (
                  <div className="flex items-center justify-center gap-2 pt-8 font-sans text-xs">
                    {currentPage > 1 ? (
                      <Link
                        href={getPageUrl(currentPage - 1, activeTab)}
                        className="p-2 border border-border hover:border-primary rounded-xl text-slate hover:text-primary transition-all"
                        aria-label="Previous Page"
                      >
                        <ChevronLeft className="w-4 h-4" />
                      </Link>
                    ) : (
                      <span className="p-2 border border-border rounded-xl text-slate opacity-40">
                        <ChevronLeft className="w-4 h-4" />
                      </span>
                    )}
                    ...
                    {Array.from({ length: totalPages }, (_, i) => i + 1).map(page => (
                      <Link
                        key={page}
                        href={getPageUrl(page, activeTab)}
                        className={`flex items-center justify-center w-8 h-8 rounded-xl font-bold transition-all border ${
                          currentPage === page
